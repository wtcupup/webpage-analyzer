...
class GetCurrentSitesTest(TestCase):
    def test_first_object(self):
        master_list=WebsiteList.objects.first()
        self.assertEqual(master_list, None) #this is true
    def do_get_current_sites(self):
        master_list=WebsiteList.objects.first()
        sites=get_current_sites(master_list) #master_list is None which is apparently a problem
        return sites
    def test_get_current_sites(self):
        sites=self.do_get_current_sites()
        self.assertEqual(sites, "") #not sure what sites should look like so running this test should show me.

class GetSiteListTest(TestCase):
    def do_get_site_list(self):
        master_list = WebsiteList.objects.first()  # what is master_list supposed to be assigned here?
        sites = get_current_sites(master_list)
        site_list = get_site_list(sites)
        return site_list
    def test_get_site_list(self):
        list=self.do_get_site_list()
